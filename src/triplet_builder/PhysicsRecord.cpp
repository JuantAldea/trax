...
void tBinnedData::operator+=(const tBinnedData c)
{
    ...
    efficiencyMean += (c.valid + c.missed) * delta / Utils::clamp((valid +
                      missed)); //mc truth defines n here
    efficiencyVar += (c.valid + c.missed) * delta * (c.efficiencyMean - efficiencyMean) +
                     c.efficiencyVar;
                             ...
                             const HitCollection::tTrackList& mcTruth, const HitCollection& hits, uint nLayerTriplets)
{
    ...
    for (uint i = tracklets.getTrackletOffsets()[event * nLayerTriplets + layerTriplet];
            i < tracklets.getTrackletOffsets()[event * nLayerTriplets + layerTriplet + 1]; ++i) {
