...
HitCollection::tTrackList HitCollection::addEvent(const PB_Event::PEvent& event,
        const DetectorGeometry & geom, EventSupplement & eventSupplement, uint evtInGroup,
        LayerSupplement & layerSupplement ,
        const TripletConfigurations & layerTriplets, int numTracks, bool onlyTracks)
{
    ...
    //found tracks initialized to 0; if no limit on track, i.e. numTracks=-1, found track will never be equal to numTracks
    int foundTracks = 0;
    
    for (tTrackList::const_iterator itTrack = allTracks.begin(); itTrack != allTracks.end()
            && foundTracks != numTracks; ++itTrack) {
    ...
    offset = evtInGroup > 0 ? eventSupplement[evtInGroup - 1].getOffset() +
             eventSupplement[evtInGroup - 1].getNHits() : 0;
