...
public:
    ...
    //quickly access both ends of track + random access
    ...
    tTrackList addEvent(const PB_Event::PEvent& event, const DetectorGeometry& geom,
                        EventSupplement & eventSupplement, uint evtInGroup, LayerSupplement & layerSupplement,
...
// create a new hit in the collection
    ...
    bool operator==(const Hit & rhs) const
    {
        return getValue<EventNumber>() == rhs.getValue<EventNumber>()
               ...
               && globalX() == rhs.globalX() && globalY() == rhs.globalY()
               && globalZ() == rhs.globalZ(); //values are only copied ==> should be bitwise identical
    }
