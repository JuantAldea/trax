...
std::string tRuntimeInfo::prettyPrint(const tRuntimeInfo & var) const
{
    ...
    s << "wall time: " << Utils::nsToMs(walltime) << " +/- " << Utils::nsToMs(
          var.walltime) << " ms"
      << " kernel time: " << Utils::nsToMs(totalKernel()) << " +/- " << Utils::nsToMs(
          var.totalKernel()) << " ms = ( "
      << Utils::nsToMs(count) << " +/- " << Utils::nsToMs(var.count) << " + " << Utils::nsToMs(
          scan) << " +/- " << Utils::nsToMs(var.scan) << "+" << Utils::nsToMs(
          store) << " +/- " << Utils::nsToMs(var.store)
      << " ) ms [Count + Scan + Store]";
...
void RuntimeRecord::logPrint() const
{
    ...
    LOG << "Wall time -- per event: " << Utils::nsToMs(totalRuntime.walltime PER events) << " ms"
        << " -- per layerTriplet: " << Utils::nsToMs(totalRuntime.walltime PER(
                    events * layerTriplets)) << " ms"
    ...
    VLOG << "Wall time -- per event: " << Utils::nsToMs(pairGen.walltime PER events) << " ms"
         << " -- per layerTriplet " << Utils::nsToMs(pairGen.walltime PER(events * layerTriplets)) <<
         " ms"
    ...
    VLOG << "Wall time -- per event: " << Utils::nsToMs(tripletPredict.walltime PER events) <<
         " ms"
         << " -- per layerTriplet " << Utils::nsToMs(tripletPredict.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(tripletPredict.walltime PER tracks) << " ms" <<
         std::endl;
    ...
    VLOG << "Wall time -- per event: " << Utils::nsToMs(tripletFilter.walltime PER events) << " ms"
         << " -- per layerTriplet " << Utils::nsToMs(tripletFilter.walltime PER(
                     events * layerTriplets)) << " ms"
    ...
    VLOG << "Wall time -- per event: " << Utils::nsToMs(tripletConnectivity.walltime PER events) <<
         " ms"
         << " -- per layerTriplet " << Utils::nsToMs(tripletConnectivity.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(tripletConnectivity.walltime PER tracks) << " ms" <<
         std::endl;

    VLOG << std::endl << "Tracklet circle fitter: ";
    VLOG << trackletCircleFitter.prettyPrint() << std::endl;
    VLOG << "Wall time -- per event: " << Utils::nsToMs(trackletCircleFitter.walltime PER events)
         << " ms"
         << " -- per layerTriplet " << Utils::nsToMs(trackletCircleFitter.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(trackletCircleFitter.walltime PER tracks) << " ms" <<
         std::endl;

    VLOG << std::endl << "Cellular automaton: ";
    VLOG << cellularAutomaton.prettyPrint() << std::endl;
    VLOG << "Wall time -- per event: " << Utils::nsToMs(cellularAutomaton.walltime PER events) <<
         " ms"
         << " -- per layerTriplet " << Utils::nsToMs(cellularAutomaton.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(cellularAutomaton.walltime PER tracks) << " ms" <<
         std::endl;
    /*********************/
}
...
void RuntimeRecordClass::logPrint() const
{
    ...
    /*****************/
    tRuntimeInfo totalRuntimeMean = buildGridMean + pairGenMean + tripletPredictMean +
                                    tripletFilterMean
    ...
    LOG << "Wall time -- per event: " << Utils::nsToMs(totalRuntimeMean.walltime PER events) <<
        " ms"
        << " -- per layerTriplet: " << Utils::nsToMs(totalRuntimeMean.walltime PER(
                    events * layerTriplets)) << " ms"
        << " -- per track: " << Utils::nsToMs(totalRuntimeMean.walltime PER tracks) << " ms" <<
        std::endl;
    ...
    VLOG << "Wall time -- per event: " << Utils::nsToMs(pairGenMean.walltime PER events) << " ms"
         << " -- per layerTriplet " << Utils::nsToMs(pairGenMean.walltime PER(
                     events * layerTriplets)) << " ms"
    ...
    VLOG << "Wall time -- per event: " << Utils::nsToMs(tripletPredictMean.walltime PER events) <<
         " ms"
         << " -- per layerTriplet " << Utils::nsToMs(tripletPredictMean.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(tripletPredictMean.walltime PER tracks) << " ms" <<
         std::endl;

    VLOG << std::endl << "Triplet filtering: ";
    VLOG << tripletFilterMean.prettyPrint(toVar(tripletFilterVar)) << std::endl;
    VLOG << "Wall time -- per event: " << Utils::nsToMs(tripletFilterMean.walltime PER events) <<
         " ms"
         << " -- per layerTriplet " << Utils::nsToMs(tripletFilterMean.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(tripletFilterMean.walltime PER tracks) << " ms" <<
         std::endl;
    /******************/
    VLOG << std::endl << "Triplet connectivity: ";
    VLOG << tripletConnectivityMean.prettyPrint(toVar(tripletConnectivityVar)) << std::endl;
    VLOG << "Wall time -- per event: " << Utils::nsToMs(tripletConnectivityMean.walltime PER
            events) << " ms"
         << " -- per layerTriplet " << Utils::nsToMs(tripletConnectivityMean.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(tripletConnectivityMean.walltime PER tracks) << " ms" <<
         std::endl;

    VLOG << std::endl << "Tracklet circle fitter: ";
    VLOG << trackletCircleFitterMean.prettyPrint(toVar(trackletCircleFitterVar)) << std::endl;
    VLOG << "Wall time -- per event: " << Utils::nsToMs(trackletCircleFitterMean.walltime PER
            events) << " ms"
         << " -- per layerTriplet " << Utils::nsToMs(trackletCircleFitterMean.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(trackletCircleFitterMean.walltime PER tracks) << " ms" <<
         std::endl;

    VLOG << std::endl << "Cellular automaton: ";
    VLOG << cellularAutomatonMean.prettyPrint(toVar(cellularAutomatonVar)) << std::endl;
    VLOG << "Wall time -- per event: " << Utils::nsToMs(cellularAutomatonMean.walltime PER events)
         << " ms"
         << " -- per layerTriplet " << Utils::nsToMs(cellularAutomatonMean.walltime PER(
                     events * layerTriplets)) << " ms"
         << " -- per track: " << Utils::nsToMs(cellularAutomatonMean.walltime PER tracks) << " ms" <<
         std::endl;
    /******************/
}
...
std::string RuntimeRecordClass::csvDump() const
{
    ...
    /***********/
    //header
    s << Utils::csv({events, layers, layerTriplets, threads, hits, tracks, (uint) records.size()})
      << SEP;
    //IO
...
void RuntimeRecordClass::addRecord(const RuntimeRecord & r)
{
    if (*this == r) {
        ...
        /************/
        calculateMeanVar(tripletConnectivityMean, tripletConnectivityVar, r.tripletConnectivity,
                         records.size());
        calculateMeanVar(trackletCircleFitterMean, trackletCircleFitterVar, r.trackletCircleFitter,
                         records.size());
        calculateMeanVar(cellularAutomatonMean, cellularAutomatonVar, r.cellularAutomaton,
                         records.size());
        /************/
...
std::string RuntimeRecords::csvDump() const
{
    ...
    s << Utils::csv({"buildGridCount", "buildGridCountVar", "buildGridScan", "buildGridScanVar", "buildGridStore", "buildGridStoreVar", "buildGridWalltime", "buildGridWalltimeVar", "buildGridKernel", "buildGridKernelVar"})
      << SEP; //buildGrid
    s << Utils::csv({"pairGenCount", "pairGenCountVar", "pairGenScan", "pairGenScanVar", "pairGenStore", "pairGenStoreVar", "pairGenWalltime", "pairGenWalltimeVar", "pairGenKernel", "pairGenKernelVar"})
      << SEP; //pairGen
    s << Utils::csv({"tripletPredictCount", "tripletPredictCountVar", "tripletPredictScan", "tripletPredictScanVar", "tripletPredictStore", "tripletPredictStoreVar", "tripletPredictWalltime", "tripletPredictWalltimeVar", "tripletPredictKernel", "tripletPredictKernelVar"})
      << SEP; //tripletPredict
    s << Utils::csv({"tripletFilterCount", "tripletFilterCountVar", "tripletFilterScan", "tripletFilterScanVar", "tripletFilterStore", "tripletFilterStoreVar", "tripletFilterWalltime", "tripletFilterWalltimeVar", "tripletFilterKernel", "tripletFilterKernelVar"})
      << SEP; //tripletFilter
    /**********/
    //tripletConnectivity
